...
describe("Tacto", () => {
    ...
    it("Should sort numbers", () => {
       expect(tacto<number>(sorters.asc(x => x))([
           4, 2, 3, 5, 9, 8
        ])).toEqual([
           2, 3, 4, 5, 8, 9
       ]);
    ...
    it("Can sort anything given a numeric value", () => {
        expect(tacto<string>(sorters.desc(x => x.charCodeAt(0)))([
            "x", "d", "e", "i", "b", "w"
        ])).toEqual([
            "x", "w", "i", "e", "d", "b"
        ]);
    ...
    it("Allows custom sort implementations", () => {
        expect(tacto<string>(sorters.raw((a, b) => a.localeCompare(b)))([
            "cat", "dog", "bird"
        ])).toEqual([
            "bird", "cat", "dog"
        ]);
    ...
    it("Can perform compound sorts", () => {
        ...
        expect(sort([
            "xta", "xvb", "xwc", "aey", "axb", "aqh"
        ])).toEqual([
            "aey", "aqh", "axb", "xwc", "xvb", "xta"
        ]);
    });
